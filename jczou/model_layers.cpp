...
bool ResumeExporter::exportToWord(const ResumePackage& package,
                                  const QString& savePath)
{
    // 1. 尝试使用 python 和 docx_generator.py 生成真正的 .docx 文件
    QString scriptPath;
    QStringList searchPaths = {
        QCoreApplication::applicationDirPath() + QStringLiteral("/docx_generator.py"),
        QCoreApplication::applicationDirPath() + QStringLiteral("/../docx_generator.py"),
        QCoreApplication::applicationDirPath() + QStringLiteral("/../../docx_generator.py"),
        QCoreApplication::applicationDirPath() + QStringLiteral("/../../../docx_generator.py"),
        QStringLiteral("docx_generator.py"),
        QStringLiteral("../docx_generator.py"),
        QStringLiteral("../../docx_generator.py")
    };
    ...
    if (!scriptPath.isEmpty()) {
        // 创建临时 json 文件，将数据包写入其中
        QString tempJsonPath = QDir::tempPath() + QStringLiteral("/temp_resume_data.json");
        if (DataManager::savePackage(tempJsonPath, package)) {
            QProcess proc;
            QString program = QStringLiteral("python");
            QStringList arguments;
            arguments << scriptPath << tempJsonPath << savePath;
    ...
    // 2. 如果 python 执行失败，则回退到原本的 HTML-based .doc 写入
    QString html = buildHtml(package);
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body {
    ...
    font-size: 14px; color: #000; line-height: 1.6; padding: 10px 0 10px 0;
  }
  .header { text-align: center; margin-bottom: 16px; }
  .name { font-size: 26px; font-weight: bold; letter-spacing: 2px; margin-bottom: 4px; }
  .contact { font-size: 13px; color: #000; }
  .job-target { font-size: 13px; color: #000; margin-top: 2px; }
  .section { margin-bottom: 16px; }
  .section-title {
    font-size: 16px; font-weight: bold; border-bottom: 2px solid #000;
    padding-bottom: 3px; margin-bottom: 8px; color: #000;
    page-break-after: auto; page-break-inside: auto; break-after: auto; break-inside: auto;
  }
  ...
  table.course-table th, table.course-table td {
    border: 1px solid #000; padding: 4px 8px; text-align: center; font-size: 13px;
    color: #000;
  }
  table.course-table th { background-color: #f2f2f2; font-weight: bold; }
  .exp-item { margin-bottom: 10px; }
  .exp-header { overflow: hidden; }
  .exp-title { font-weight: bold; font-size: 15px; float: left; color: #000; }
  .exp-org { float: left; margin-left: 8px; color: #000; }
  .exp-date { float: right; font-size: 13px; color: #000; }
  .exp-desc { clear: both; margin-top: 4px; font-size: 13px; color: #000; white-space: pre-wrap; }
  .gpa-highlight { font-weight: bold; color: #000; font-size: 16px; }
  .skills-block { font-size: 13px; line-height: 1.8; white-space: pre-wrap; color: #000; }
